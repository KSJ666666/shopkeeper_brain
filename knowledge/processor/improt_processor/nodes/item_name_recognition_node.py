...
class ItemNameRecognitionNode(BaseNode):
    ...
    def _recognition_item_name(self, item_name_context: str, file_title: str) -> str:
        ...
        # 1.调用LLM客户端
        try:
            llm_client: ChatOpenAI = AIClients.get_openai_llm_client(response_format=True)
        ...
        try:
            ...
            # 5. 解析 JSON 输出（response_format=json_object 模式下 LLM 返回 JSON 对象），提取 product_name
            try:
                parsed = json.loads(llm_result)
                if isinstance(parsed, dict) and 'product_name' in parsed:
                    llm_result = str(parsed['product_name']).strip()
...
if __name__ == '__main__':
    ...
    # 1. 读取chunk.json
    temp_dir = Path(
        r"D:\code\shopkeeper_brain\docs\temp_dir")
